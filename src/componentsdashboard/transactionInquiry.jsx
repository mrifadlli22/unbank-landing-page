...
function TransactionIqr() {
  ...
  const [isDataVisible, setIsDataVisible] = useState(true); // State to control table visibility
  ...
  const clearFilters = () => {
    ...
    setEndDate("");
    setSearchTerm("");
    setWarning(""); // Clear warning when filters are reset
    setIsDataVisible(true); // Reset table visibility
  };
  ...
  const handleDateChange = (type, value) => {
    ...
    if (start && end) {
      ...
      if (diffDays > 7) {
        setWarning("Maksimal filter waktu selama 7 hari!");
        setIsDataVisible(false); // Hide data when date range exceeds 7 days
        return; // Return to prevent further execution
      } else {
        setWarning(""); // Clear warning if within 7 days
        setIsDataVisible(true); // Show data
      }
    }
  };
